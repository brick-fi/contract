...
contract PropertyTokenTest is Test {
    ...
    function test_DistributeRevenue() public {
        // Setup: Two investors with different holdings
        vm.startPrank(investor1);
        usdc.approve(address(token), 102 * 1e6); // 100 USDC investment + 2% fee
        token.invest(102 * 1e6); // 2 tokens
        vm.stopPrank();

        vm.startPrank(investor2);
        usdc.approve(address(token), 51 * 1e6); // 50 USDC investment + fee
        token.invest(51 * 1e6); // 1 token
        vm.stopPrank();

        // Admin distributes revenue
        // Expected 100 USDC for all tokens (maxSupply = 2000)
        // Sold: 3 tokens out of 2000 = 0.15%
        // Actual amount admin transfers: 100 * (3/2000) = 0.15 USDC
        uint256 expectedAmount = 100 * 1e6; // 100 USDC
        uint256 soldTokens = token.getSoldTokens(); // 3 tokens
        uint256 maxSupply = token.maxSupply(); // 2000 tokens
        uint256 actualAmount = (expectedAmount * soldTokens) / maxSupply; // 150,000 (0.15 USDC)
        ...
        // Verify admin only paid actual amount
        assertEq(adminBalanceBefore - usdc.balanceOf(admin), actualAmount);

        // Verify investors received their shares (push-based automatic transfer)
        uint256 investor1Share = (actualAmount * 2) / 3; // 2/3 of actual amount
        uint256 investor2Share = (actualAmount * 1) / 3; // 1/3 of actual amount
        assertEq(usdc.balanceOf(investor1) - investor1BalanceBefore, investor1Share);
        assertEq(usdc.balanceOf(investor2) - investor2BalanceBefore, investor2Share);
    }
